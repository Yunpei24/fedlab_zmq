...
class DeviceProfile:
    ...
    def flops_for_model(self, num_params: int, batch_size: int,
                        local_epochs: int, dataset_size: int,
                        training_flop_factor: int = 3) -> float:
        """
...
def _channel_params():
    ...
    try:
        from hardware.energy_model import (
            CHANNEL_WIFI5_2G, CHANNEL_WIFI4_2G,
            CHANNEL_ESP32, CHANNEL_LTE, CHANNEL_5G, CHANNEL_ETHERNET,
        )
...
def list_profiles() -> None:
    ...
    print(f"{'Name':<30} {'Peak GFLOPS':>12} {'RAM MB':>8} "
          f"{'↑Mbps':>8} {'↓Mbps':>8} {'Battery J':>10}")
    print("─" * 80)
    for key, p in DEVICE_PROFILES.items():
        print(f"{p.name:<30} {p.compute.peak_gflops:>12.1f} "
