#!/usr/bin/env python3
"""V6 fixed-step ablation of the split risk/gradient clean MPS calibration."""
import argparse
...
NAME='split_risk_gradient_fixed_step_v6'
MATRIX=ROOT/'configs/ldp_gradient_far'/f'{NAME}.yaml'
OUT=ROOT/'results/ldp_gradient_far'/NAME
LOG=ROOT/'logs'/f'{NAME}.log'
REPORT=ROOT/'output/analysis/Split_Risk_Gradient_Fixed_Step_V6_Status.md'
TESTS=['tests/test_fair_objective.py','tests/test_split_risk_gradient.py','tests/test_split_risk_gradient_fixed_step.py']
...
def config():
    ...
    assert m['eta_erm']==m['eta_fair']==2 and m['parent_campaign']=='split_risk_gradient_v5'
...
def report(m,stamp):
    ...
    lines=['# Risque privé + gradient privé — ablation du pas fixe v6','',f'**{len(rows)}/10 runs valides**, MPS, validation uniquement.',
        '', '60 tours, 10 clients, batch 240, C=1, epsilon réalisé ≈ 4 ; risques frais à chaque tour, pas fixe 2 pour toutes les règles. '
        'Aucune attaque et aucune évaluation test. Pas de conclusion avant les dix runs.', '',
        '[Protocole](Split_Risk_Gradient_Fixed_Step_V6_Protocol.md)', '',
    ...
    if len(rows)==10:
        ...
        lines+=['','Deux seeds de calibration : même en cas de réussite, aucune validation finale ni robustesse démontrée.', '',
                 '[Evidence](../../results/ldp_gradient_far/split_risk_gradient_fixed_step_v6/evidence.json)']
...
def worker(m):
    ...
    with lock():
        try:
            from scripts import run_split_risk_gradient_v5 as parent
            ...
            assert (ROOT/m['paired_randomness_source']).is_file()
            ...
            if not secret.exists():
                parent_key=json.loads((ROOT/m['paired_randomness_source']).read_text())['key']
                base.save(secret,dict(key=parent_key,not_public=True,paired_with=m['parent_campaign']))
            assert json.loads(secret.read_text())['key']==json.loads((ROOT/m['paired_randomness_source']).read_text())['key']
            ...
            assert count==10
            # Identical-method ERM replay and identical batch patterns across the version change.
            for seed in m['calibration_seeds']:
                a=json.loads((OUT/identifier(dict(seed=seed,method='erm_full'))/'metrics.json').read_text())
                b=json.loads((parent.OUT/identifier(dict(seed=seed,method='erm_full'))/'metrics.json').read_text())
                for metric in ('accuracy_pct','worst20_pct','brier_loss'):
                    assert abs(a['final']['validation'][metric]-b['final']['validation'][metric])<1e-6
                for method in m['methods']:
                    relative=identifier(dict(seed=seed,method=method))
                    aa=json.loads((OUT/relative/'simulator_oracle.json').read_text())
                    bb=json.loads((parent.OUT/relative/'simulator_oracle.json').read_text())
                    assert [[c['batch_hash'] for c in t['clients']] for t in aa['rounds']]==[[c['batch_hash'] for c in t['clients']] for t in bb['rounds']]
            base.save(OUT/'parent_pairing_audit.json',dict(passed=True,erm_reproduced=True,batches_paired=True))
