...
OUT=ROOT/'results/ldp_gradient_far/fresh_private_risk_step_v22'
...
REPORT=ROOT/'output/analysis/Fresh_Private_Risk_Step_V22_Status.md'
TEST=ROOT/'tests/test_fresh_private_risk_step_v22.py'
...
def inputs():
    config=yaml.safe_load(MATRIX.read_text());assert config['campaign_id']==OUT.name
    ...
    files=[Path(__file__),MATRIX,PROTOCOL,TEST,a]
...
def train(j,config,profile,stamp,data,key):
    ...
    assert p==reference['privacy'];expected=json.loads((prior.OUT/f'seed{j["seed"]}__fresh__{j["method"]}/simulator_oracle.json').read_text())['rounds']
