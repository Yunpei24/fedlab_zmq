...
def test_phi_matches_legacy_fedpart(resnet8, resnet8_groups,
                                    rpi4_profile, cifar_loader, fp_mod):
    from hardware.flop_cost import round_compute_flops
    cfg = {"cost_model": "phi", "input_shape": (1, 3, 32, 32)}
    gf = fp_mod._compute_group_flops(resnet8_groups, resnet8, input_shape=(1, 3, 32, 32))
    num_params = sum(p.numel() for p in resnet8.parameters())
    full = rpi4_profile.flops_for_model(num_params, 32, 1, 128)
    for g_idx in range(len(resnet8_groups)):
        got = round_compute_flops(
            resnet8, resnet8_groups[g_idx], cfg,
            rpi4_profile, cifar_loader, 1,
            groups=resnet8_groups, active_group_idx=g_idx,
...
def test_corrected_shallow_costs_more_than_phi_shallow(
    ...
    common = dict(
        groups=resnet8_groups, active_group_idx=0,
        group_flops_analytic=gf,
    )
    phi = round_compute_flops(
        resnet8, resnet8_groups[0], {**base, "cost_model": "phi"},
        rpi4_profile, cifar_loader, 1, **common,
    )
    corr = round_compute_flops(
        resnet8, resnet8_groups[0], {**base, "cost_model": "corrected"},
        rpi4_profile, cifar_loader, 1, **common,
    )
    assert corr > phi
...
def test_corrected_falls_through_to_measured_for_scattered_set(
    ...
    from hardware.flop_cost import (
        cache_stats, clear_cache, round_compute_flops,
    )
    clear_cache()
...
def test_fedpart_helpers_still_callable(fp_mod, resnet8, resnet8_groups):
    ...
    from hardware.flop_cost import (
        compute_group_flops as _canonical_group_flops,
        compute_corrected_group_costs as _canonical_corrected,
    )
    via_fedpart = fp_mod._compute_group_flops(
        resnet8_groups, resnet8, input_shape=(1, 3, 32, 32)
    )
    direct = _canonical_group_flops(
        resnet8_groups, resnet8, input_shape=(1, 3, 32, 32)
    )
